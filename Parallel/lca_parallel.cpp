...
string out_file = "../io_files/outputs/parallel/large.out";
...
void build_rmq(vector < vector<int> > &rmq, vector <int> &lg,
    ...
    for (int i = 1; i <= lg[tour_length]; ++i) {
        int j;
        #pragma omp parallel for private(j) schedule (SCHEDULE)
...
int main() {
    ...
    // Answering queries in parallel
    {
        int i;
        #pragma omp parallel for private(i) schedule (SCHEDULE)
        for (i = 1; i <= q; ++i) {
